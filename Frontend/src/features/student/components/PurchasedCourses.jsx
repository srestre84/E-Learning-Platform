...
import {
  ...
  getEnrolledCourses,
} from "@/services/enrollmentService"; // ✅ Importamos los servicios de la API
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        console.log("Error al cargar tus cursos:", e);
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[...Array(3)].map((_, index) => (
          <div
            key={index}
            className="rounded-lg overflow-hidden shadow-md animate-pulse">
